...
public class MalePathGen {
  ...
  public static String hingedGen(
        double length,
        double toothWidth,
        double depth,
        int dx, int dy,
      ...
      // ---- left corner offset ----
      sb.append(rel(dx * (leftCornerTravel - cornerKerf - hingeRM), dy * (leftCornerTravel - cornerKerf - hingeRM)));
      hingeRM = (panel.role == Panel.PanelRole.leftBottom) ? 3.175 : 0;
      ...
      // ---- right corner offset ----
      sb.append(rel(dx * (rightCornerTravel - cornerKerf - hingeRM), dy * (rightCornerTravel - cornerKerf - hingeRM)));
  ...
  public static String cutOutGen(
        double length,
        double toothWidth,
        double depth,
        int dx, int dy,
      ...
      // Effective corner travel on each side after accounting for neighbor male-depth retraction.
      double leftCornerTravel  = corner - (lastIsMale ? depth + 3 : 0.0) - ((panel.role == Panel.PanelRole.leftTop) ? 8 : 0) + (nextIsMale ? 3 : 0);
      double rightCornerTravel = corner - (nextIsMale ? depth + 3 : 0.0) - ((panel.role == Panel.PanelRole.rightTop) ? 8 : 0) + (lastIsMale ? 3 : 0);
      ...
      if (panel.role == Panel.PanelRole.leftTop) {
      ...
      if (panel.role == Panel.PanelRole.rightTop) {
        sb.append(rel(px * depth, py * depth));
        sb.append(relA(-8, 8, 0));
      } 
